    ...
    <div style={{background: '#fff', borderRadius: 8, boxShadow: '0 2px 8px #0001', padding: 16, marginBottom: 24}}>
      ...
      <table style={{width: '100%', borderCollapse: 'collapse'}}>
        ...
        <tbody>
          {filtered.map(j => (
            <tr key={j.id}>
              <td><a href="#" onClick={e => {e.preventDefault();setSelectedJob(j.id);}}>{j.id}</a></td>
              <td>{j.type}</td>
              <td>{j.priority}</td>
              <td>{j.attempts}</td>
              ...
              <td>{badge(j.state)}</td>
              <td>
                {j.state==='failed' && <button onClick={() => handleRetry(j.id)} disabled={loading} style={{background:'#d32f2f',color:'#fff',border:'none',borderRadius:4,padding:'4px 8px',cursor:'pointer'}}>Retry</button>}
                {j.state==='dead' && <button onClick={() => handleRequeue(j.id)} disabled={loading} style={{background:'#616161',color:'#fff',border:'none',borderRadius:4,padding:'4px 8px',cursor:'pointer'}}>Requeue</button>}
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {selectedJob && <JobTimeline jobId={selectedJob} onClose={() => setSelectedJob(null)} />}
    </div>
  );
}
